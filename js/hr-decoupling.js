...
function _hrdBand(dc) {
  if (dc < 0)   return { color: '#4da8ff', text: 'Negative drift — you held or raised output as HR settled (negative split or long warm-up). Strong aerobic control.' };
  if (dc <= 5)  return { color: '#22c55e', text: 'Well-coupled (<5%). Strong aerobic endurance for this effort — HR stayed steady against your output.' };
  if (dc <= 10) return { color: '#fb923c', text: 'Moderate drift (5–10%). Normal for a hard or long ride; watch fuelling and pacing on the back half.' };
  return { color: '#ef4444', text: 'High decoupling (>10%). Aerobic endurance, pacing, heat or fuelling limited the second half — a target to build.' };
}
...
function _hrdMarkup(a, d) {
  ...
  const half = (n, hr, o) => `<div class="hrd-half">
    <div class="hrd-half-lbl">${n} half</div>
    <div class="hrd-half-row"><span>${Math.round(hr)} bpm</span><span>${outVal(o)}</span></div>
  </div>`;
  return `
    <div class="hrd-grid">
      ${half('First', d.hr1, d.o1)}
      <div class="hrd-dc"><div class="hrd-dc-val" style="color:${band.color}">${dcStr}</div><div class="hrd-dc-lbl">decoupling</div></div>
      ${half('Second', d.hr2, d.o2)}
    </div>
    <div class="tr-basis-note">${band.text}</div>
    <div class="tr-basis-note">Latest ride: ${a.name || 'Ride'} · ${d.usePower ? 'power' : 'speed'} vs HR.</div>`;
}
...
function _trHrDecouplingHTML() {
  ...
  return `<div class="card tr-hrd">
    <div class="tr-chart-title">Heart Rate Decoupling <span class="gm-hint">aerobic drift, first vs second half</span></div>
    <div id="hrdBody">
      <div class="tr-basis-note">See how much your heart rate drifts up relative to your pace/power over a long ride.</div>
      <button class="tr-ai-btn" style="margin-top:10px" onclick="analyzeHrDecoupling()">Analyse ${a.name || 'latest ride'}</button>
    </div>
  </div>`;
}
...
async function analyzeHrDecoupling() {
  ...
  const a = _hrdPickRide();
  if (!a) { body.innerHTML = '<div class="tr-basis-note">No long ride with heart-rate data yet.</div>'; return; }
  _hrdRunning = true;
  ...
  if (!streams) { body.innerHTML = '<div class="tr-basis-note">Couldn\'t load stream data for this ride.</div>'; _hrdRunning = false; return; }
  const d = _hrdCompute(streams);
  body.innerHTML = d ? _hrdMarkup(a, d) : '<div class="tr-basis-note">Not enough continuous HR/output data in this ride to measure drift.</div>';
  _hrdRunning = false;
}
